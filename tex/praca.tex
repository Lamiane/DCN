...
\begin{document}
  ...
  \chapter{Introduction}
    ...
    \todo[inline]{obrazek z zadokowanym ligandem}
    ...
    As already said, there are many different fingerprints designed. They vary in length and the features included. In this paper we used 2D SIFt~\cite{2DSIFT} fingerprints. % TODO why this one not the other one?
    ...
    \igor{
    These two models added result in a $6\times{}9$ matrix for each possible point of contact. The model shall now be called 2D-SIFts~\cite{Mordalski2011}. For a given protein and given molecule this results in a string of $6\times{}9$ arrays glued together. We can say, that this representation resembles and image, where patterns shall be looked after. This explains why a convolutional network is used in this work.
	}
  ...
  \igor{In this chapter the Convolutional Neural Network (CNN) model shall be described.}
     ...
     \begin{figure}[h!]
	  ...
	  \caption{An example of convolutional neural network. The figure was downloaded from http://parse.ele.tue.nl/education/cluster2} %TODO poprawić link!
	...
	\subsection{Summary}
	  ...
	  % FIXME can the pattern be *discovered* by the filter? recognise?
