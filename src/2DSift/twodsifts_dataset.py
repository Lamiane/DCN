...
class TwoDSiftData(DenseDesignMatrix):
    def __init__(self, filenames=[], y_val=[], nogap_type=True, replicate=0, labels=False, shuffle=True,
        ...
        # TODO remember filenames as a dictionary with additional informations, like number of examples, etc.
        """
        ...
        """
        # print ">>>>>>>>>> TwoSiftData.__init__(filenames =", filenames, ", y_val =", y_val, ", replicate =", replicate, \
        # ", cv =", cv, ")"
        ...
        if nogap_type:
            ...
            topo_view = self.read()
        # print len([item for sublist in skipped for item in sublist]), "examples skipped:", skipped
        # TODO check the assert together with number of skipped records
        # assert self.examples == len(self.ligands), self.__class__.__name__ + ": number of examples (" + str(self.examples) + \
        # ") returned not equal to number of ligand keys (" + str(len(self.ligands)) + ")!"
        axes = ('b', 0, 1, 'c')
        ...
        if cv is not None and isinstance(cv, list):
            if len(cv) != 2:
                print "cv split: incorrect cv parameter list:", cv
            elif not isinstance(cv[0], int) or cv[0] < 2:
                print "cv split: incorrect number of parts to split:", cv[0]
            # elif not isinstance(cv[1], list) or len(cv[1]) < 1 or len(cv[1]) > cv[0] - 1:
            #     if len(cv[1]) == 0:
            #         self.splits = [int((self.examples / float(cv[0])) * i) for i in range(cv[0])]
            #     print "cv split: incorrect list of parts to choose from: ", cv[1], "( cv[0] =", cv[0], ")"
            elif len(cv[1]) == 1 and (cv[1][0] < 0 or cv[1][0] >= cv[0]):
                print "cv split: incorrect split to choose cv[1] =", cv[1][
                    0], "in relation to number of splits cv[0] =", \
                    cv[0]
            elif len(cv[1]) == cv[0] - 1 and any([item < 0 or item >= cv[0] for item in cv[1]]):
                print "cv split: incorrect splits to choose cv[1] =", cv[1], "in relation to number of splits cv[0] =", \
                    cv[0]
            elif len(cv[1]) == cv[0] - 1 and len(cv[1]) > len(set(cv[1])):
        ...
        super(TwoDSiftData, self).__init__(topo_view=topo_view, y=y, axes=axes, y_labels=self.n_classes)
